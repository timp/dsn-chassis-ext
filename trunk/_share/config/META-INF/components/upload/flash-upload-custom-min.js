...
(function()
{
   ...
   YAHOO.extend(Alfresco.FlashUpload, Alfresco.component.Base,
   {
      ...
      _createPopulatedDerivedFilesDataTable: function FlashUpload__createPopulatedDerivedDataTable()
      {
         ...
         this.derivedDataTable = new YAHOO.widget.DataTable(dataTableDiv, myColumnDefs, myDataSource, myConfigs);

         this.derivedDataTable.on('checkboxClickEvent', function (oArgs) {
              var target = oArgs.target;
              var record = this.getRecord(target);
              var column = this.getColumn(target);
              if (column.key == 'select') { // to ensure we respond to the right column, if more checkboxes are added in the future
